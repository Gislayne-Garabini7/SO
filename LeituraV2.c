...
// Função para obter atributos do arquivo (FUSE)
static int myfs_obter_atributos(const char *caminho, struct stat *stbuf) {
    int resultado = 0;

    // Obtém informações do arquivo usando stat
    if (stat(caminho_arquivo, stbuf) == -1) {
        resposta = -errno; // Erro ao obter informações do arquivo
        return resposta;
    }
